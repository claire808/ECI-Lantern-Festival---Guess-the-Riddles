...
const OracleSpread: React.FC<{ 
  ...
  return (
    <div className="relative w-full h-[320px] flex items-center justify-center mb-4 z-20">
      {cards.map((idx) => {
        ...
        return (
          <div
            ...
            `}
            style={{
              width: isSelected ? '220px' : '110px',
              height: isSelected ? '310px' : '180px',
...
const AnswerTarot: React.FC<{
  ...
  return (
    <div 
      className={`group h-14 w-full perspective-1000 cursor-pointer mb-2 ${disabled ? 'cursor-default' : ''}`}
...
const App: React.FC = () => {
  ...
  const shareResults = () => {
    const text = `🏮 I found ${score}/12 truths in ECI Oracle! 🎴 #ECI #LanternFestival`;
    if (navigator.share) {
